...
import com.estudoAPI.pontointeligente.api.dtos.FuncionarioDto;
	...
	private void atualizarDadosFuncionario(Funcionario funcionario,FuncionarioDto funcionarioDto, BindingResult bindingResult) throws NoSuchAlgorithmException{
		...
		if(!funcionarioDto.getEmail().equals(funcionario.getEmail())) {
			this.funcionarioService.buscarPorEmail(funcionarioDto.getEmail())
				.ifPresent(func -> bindingResult.addError(new ObjectError("email", "Email já existe na base")));
		}
		
		
		funcionario.setQtdHorasAlmoco(null);
		funcionarioDto.getQtdHorasAlmoco()
			.ifPresent(qtdHorasAlmocoDia-> funcionario.setQtdHorasAlmoco(Float.valueOf(qtdHorasAlmocoDia)));
		
		funcionario.setQtdHorasTrabalhoDia(null);
		funcionarioDto.getQtdHorasTrabalhoDia()
			.ifPresent(qtdHorasTrab -> funcionario.setQtdHorasAlmoco(Float.valueOf(qtdHorasTrab)));
		
		funcionario.setValorHora(null);
		funcionarioDto.getValorHora().ifPresent(valorHora -> funcionario.setValorHora(new BigDecimal(valorHora)));

		if (funcionarioDto.getSenha().isPresent()) {
			funcionario.setSenha(PasswordUtils.gerarBCryp(funcionarioDto.getSenha().get()));
		}
